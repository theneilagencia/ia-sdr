...
def verify_production_secrets() -> None:
    ...
    publica = urlparse(settings.public_base_url)
    if publica.hostname in {"localhost", "127.0.0.1", "0.0.0.0", None}:
        problemas.append(
            f"PUBLIC_BASE_URL aponta para {settings.public_base_url}, que não "
            "existe fora do servidor. É a base do link de descadastro que vai "
            "em todo email: errada, o link não abre — e email frio sem saída "
            "queima o domínio da empresa."
        )
    elif publica.scheme != "https":
        problemas.append(
            f"PUBLIC_BASE_URL usa {publica.scheme}://. O link de descadastro "
            "chega por email para gente de fora; sem TLS ele vai em claro."
        )
